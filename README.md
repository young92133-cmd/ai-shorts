...
| 단계 | 하는 일 |
|---|---|
| ① 프로젝트 목록 | 만든 영상 목록. 눌러서 이어서 작업, 「새로 만들기」 |
| ② 소재 | 주제 / 링크 / 영상 업로드 / 핫이슈, 카테고리, 내 파일 첨부 |
| ③ 스타일·지침 | 벤치마킹 스타일, 이번 영상 지침, 대본 AI, 자막 넣기 여부 → **대본 만들기** |
| ④ 대본 | 나레이션·키워드 수정, 쓸 영상·댓글 후보 고르기 |
| ⑤ 음성 | TTS 종류와 목소리 |
| ⑥ 화면 배치 | 장면별로 Flow 이미지 등 올리기 → **렌더링 시작** |
| ⑦ 자막·댓글 | (선택) 자막 문구·시간 수정, 자막 끄기, 댓글 캡처 끌어다 놓기 → **적용해서 다시 만들기** |
| ⑧ 내보내기 | MP4 다운로드, 제목·설명 복사, **CapCut 프로젝트로 보내기**, 재료 묶음(zip) |

- ⑦은 모두 선택 사항입니다. 고칠 게 없으면 「건너뛰기」를 누르세요. 다시 만들기는 음성·대본을 그대로 두고 영상만 다시 렌더하므로 비용이 없습니다(1~3분).
- CapCut으로 보내면 장면·나레이션·배경음악·자막·키워드·댓글이 각각 따로 된 층으로 들어갑니다. CapCut이 켜져 있었다면 껐다 켜야 목록에 보입니다. 확대 효과·장면 전환·노래방식 자막 강조는 옮겨지지 않습니다.
- 스타일을 만들 때 「댓글 캡처가 언제·어디에 뜨는지도 분석」을 켜면 참고 영상의 댓글 위치를 배워 두었다가, 그 스타일로 만들 때 댓글을 같은 흐름으로 자동 배치합니다(Gemini 또는 OpenAI 키 필요, 사용량 요금 발생 가능).
